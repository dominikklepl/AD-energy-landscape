...
plot_basin_voronoi = function(basin_df, states_df) {
...
plot_basin = function(basin_df, states_df) {
  ...
  plot = ggraph(tidy)+
    geom_edge_link()+
    geom_node_point(aes(color=energy, shape = minimum, size = minimum))+
...
analyze_energy_landscape = function(file){
  ...
  epoch = strsplit(strsplit(fname, "_")[[1]][4], "\\.")[[1]][1]
  
  info = data.frame(ID, diagnosis, condition, epoch)
...
collect_parameters = function(files){
  J = list()
  h = list()
  for(i in 1:length(files)){
    load(files[i])
    J[[i]] = results$parameters$J
    h[[i]] = results$parameters$h
  }
  return(list(h=h,J=J))
}

average_params = function(params){
  J_dims = dim(params$J[[1]])
  J_len = length(params$J)
  J = array(unlist(params$J), c(J_dims, J_len))
  
  h_dims = dim(params$h[[1]])
  h_len = length(params$h)
  h = array(unlist(params$h), c(h_dims, h_len))
  
  J_avg = rowMeans(J, dims = 2)
  h_avg = rowMeans(h, dims = 2)
  
  return(list(h = h_avg, J=J_avg))
}
