...
if(sliding==T){
  #sliding binarisation
  binarize = function(data, size){
      data$window = ceiling(seq_along(data[,1])/size)
      threshold = function(x){return(ifelse(x<=mean(x),-1,1))}
      data = data %>% group_by(window) %>% mutate_all(list(threshold)) %>% ungroup() %>% select(-window)
      return(t(data))
    }
} else{
    binarize = function(data){
      data = t(data)
      data = apply(data, 1, function(x) ifelse(x >= mean(x), 1, -1))
      return(t(data))
    }
}

pMEM_PL = function(data, lr_rate = 0.2, iter_max = 5000, stopping = 1e-9) {
  len = ncol(data)
  N = nrow(data)
...
run_pMEM = function(data, size){
  df = binarize(data, size)
  params = pMEM_PL(df)
...
run_folder = function(wave_band, selection, selection_name){
  ...
  pMEM_results = matrix(NA, length(files), 6)
  colnames(pMEM_results) = c("ID", "diagnosis", "condition", "epoch", "r", "R2") #29
  for(i in 1:length(files)){
    ...
    results_row = cbind(ID, diagnosis, condition, epoch, results$metrics$r, results$metrics$R2)
