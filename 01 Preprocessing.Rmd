...
```{r}
selected_channels = c("P3_O1", "T6_O2", "P4_PZ", "T3_T5", "T3_C3", "F3_FZ", "F4_C4")

selected_df = locs
selected_df$label = row.names(selected_df)
selected_df = selected_df[selected_df$label %in% selected_channels,]

(channels_plot = geom_head()+
  geom_point(data=selected_df, aes(x,y),size=16, color="navy")+
  geom_text(data=selected_df, aes(x,y, label=label), color="white"))

ggsave("figures/overview_montage.png", channels_plot)
```
